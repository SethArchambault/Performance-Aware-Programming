...
// I hate that these are all strings
...
reg get_reg_from_str(u8 rm) {
    if (strcmp(effective_address[rm], "bx") == 0) return bx;
    if (strcmp(effective_address[rm], "bp") == 0) return bp;
    if (strcmp(effective_address[rm], "di") == 0) return di;
    if (strcmp(effective_address[rm], "si") == 0) return si;
    ...
    switch(inst.type) {
        ...
        case t_mov_immed_to_reg: 
        {
            ...
            if (before != after) {
                snprintf(debug_str, 100, "%s %s:0x%hx->0x%hx", 
                    debug_str, registers[reg][w], before, after);
            }
            break; 
        }
        case t_asc_immed_to_reg_mem:
        {
            ...
            if (old_flags[0] || new_flags[0]) {
                snprintf(debug_str, 100, "%s %s flags:%s->%s", 
                        debug_str, debug_str, old_flags, new_flags);
            }
        ...
        case t_immed_to_reg_mem:
        {
            if (mode == mode_effective_addr_8bit_mem) {
                // works!
                if (strcmp(effective_address[rm], "bx + si") == 0) assert(0);
                if (strcmp(effective_address[rm], "bx + di") == 0) assert(0);
                if (strcmp(effective_address[rm], "bp + si") == 0) assert(0);
                if (strcmp(effective_address[rm], "bp + di") == 0) assert(0);
                u8 effective_reg = get_reg_from_str(rm);
                memory[reg_value[effective_reg] + mem] = immed & 0x00ff;
            } else if (mode == mode_16bit_mem) {
                memory[mem] = immed;
            } else if (mode == mode_effective_addr) {
                if (strcmp(effective_address[rm], "bx") == 0) {
        ...
        case t_reg_to_reg_mem:
        {
            if(mode == mode_effective_addr_8bit_mem) {
                ...
                // maybe don't store these in strings. this is stupid.
                if (strcmp(effective_address[rm], "bp + si") == 0) {
                    if (!d) {
                        memory[reg_value[bp] + reg_value[si]] = reg_value[reg] & 0x00ff;
//                        memory[reg_value[bp] + reg_value[si] +1] = reg_value[reg] >>8;
                    } else {
                        reg_value[reg] = memory[reg_value[bp] + reg_value[si]];
 //                       reg_value[reg] |= memory[reg_value[bp] + reg_value[si]+1]<<8;
                    }
                } else {
                    assert(0);
                }
